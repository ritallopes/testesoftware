...
public class MainFrame extends javax.swing.JFrame {
	...
	private TableModel tableListConsultaModel;
	private TableModel tableListagemModel;
			...
			btnCadastrar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
					...
					try {
						...
						for(UfDTO uf : ufBO.listaUfs()) {
							if(uf.getIdUF().equals(comboUf.getSelectedIndex() + 1)) {
								ufDTO = uf;
								break;
							}
						}
						...
						//main(null);
						System.out.println("idsPessoas = " + idsPessoas);
						try {
							String[][] lista = pessoaBO.listagem(idsPessoas, pessoaDAO);
							...
							tableListagemModel = 
									new DefaultTableModel(
											lista,
											new String[] { "Id", "Nome", "CPF", "Sexo", "Dt. Nasc.", "Logradouro", "UF", "CEP", "", "" });
			...
			Action actionDelecao = new AbstractAction() {
				...
				public void actionPerformed(ActionEvent actionEvent) {
					...
					if (resp == 0) {
						...
						try {
							pessoaDTO = pessoaBO.buscaPorId(idsPessoas.get(linha), pessoaDAO);

							pessoaBO.removePessoa(idsPessoas.get(linha), getPessoaDTO().getEnderecoDTO().getIdEndereco(), pessoaDAO);
